# Improvement Changelog — unabridged

The full working record: every experiment, written the day it ran, with the
evidence tables in full. The abridged version a reader should start from is
[`CHANGELOG.md`](../CHANGELOG.md), which carries the same numbers and the same
decisions in a tenth of the words.

Entries keep their original numbers. Work on the terminal interface changed what
a person sees without moving a benchmark number, so it lives in
[`INTERFACE-LOG.md`](INTERFACE-LOG.md).
...
## Iteration 3, completed — the full sweep (2026-08-29)

> Headed **Final** until the numbering was tidied, which made a mid-document
> entry look like the end of the story and invited a reader to take the F1 table
> below as the result. It is not: F1 was retired as the primary metric two
> entries later, for the reason given there. The final result is at the
> [end of this file](#the-main-failure-mode).
...
The safeguards in this project are all pointed at the agent. Evidence passes one
provider that asserts the holdout on every record. Stage D drops a finding whose
citation does not resolve, and since Iteration 24 drops one whose quotation is
not in the thread it cites. Replay refuses to serve a recording whose prompt has
changed. Every one of those guards works, and each is covered by a test.

None of them guard the prose. A number written into the README on the day it was
measured stays there after the thing it measured is re-run, and no test fails.
That is how the repository came to state its own verdict stability three
different ways at once — 21/22 in one file, 55/55 in another, and 22/22 in the
tool's actual output — and how a reproduction guide came to promise a test count
that had been wrong for weeks. The measurements were right every time. The
sentences about them rotted, silently, because a sentence is not an assertion.

The same shape produced the worst defect a judge could have hit: the documented
command for the baseline solution, `holt analyze <repo> --baseline --replay`,
failed on every repository from a clean clone. Baseline calls were recorded only
under the run-tagged directories the harness reads, so nothing under
`fixtures/trajectories/` could answer one. The benchmark was never affected — it
reads the tagged runs — which is exactly why nobody noticed. The evaluation and
the documented product path had drifted apart and only the evaluation was tested.
...
`ASSESSMENT.md` disagrees; it parses `REPRODUCTION.md` for the commands it
tells a judge to run and runs each one, including under a model chosen with
`holt models`; and it checks the promised test count against what the suite
actually collects. That is 21 new tests (**339 passed**, no skips, with the
optional TUI extra installed; 266 without it). The three claims that had rotted
— `21/22` and `13/22` for verdict stability, `128 passed`, `115 passed` — are
corrected to `22/22`, `17/22`, `266`/`339`, and a stale sentence is now a
failing test rather than a thing a reader finds first.

**The two defects themselves.** `scripts/backfill_baseline_trajectories.py`
files the already-recorded baseline calls where `model.build` looks for them —
nothing is re-recorded, no model runs, and the keys match because the recording
always answered this exact question. And replay now resolves against the pinned
defaults regardless of what `holt models` has been asked for, which is what
`model.py` already claimed in its own docstring: a recording is reproduced under
the ids that made it, and a reader's model choice governs calls that actually
reach a provider. Before this, choosing a model turned six of the seven
documented commands into a replay miss.
...
prompt handed the same evidence matches the model stages almost exactly, and
ablating Stage A's kind rules costs +0.01 MCC. What actually separates Holt from
a chat window is duller than intelligence and harder to fake: an evidence
assembly nobody will do by hand — a median of 642 records and 253,000 characters
per repository, 44× what a person can paste — and then three properties a
conversation structurally cannot have. Every claim carries an id that resolves
to a real thread. The verdict is a plain function, so it returned the identical
answer on 55 of 55 repositories across three runs while the one-prompt baseline
moved on 16. And it can say no, in a written rule the model is unable to
override, which is the single change that most improved accuracy — specificity
0.58 → 0.83 out of sample.

**What we would build differently next time:** put the decision in code and the
model in front of the evidence, not the other way round. Then spend the saved
effort on the boring half — because the lesson of the failure mode above is that
an agent's guarantees are worth exactly as much as the tests on the claims you
make about them, and prose is where reliability goes to die.
