...
One entry per experiment, and nothing else. Work on the terminal interface
changed what a person sees without moving a benchmark number, so it lives in
[`docs/INTERFACE-LOG.md`](docs/INTERFACE-LOG.md) rather than diluting the record
of what was measured. The failure mode and the hot take are at the
